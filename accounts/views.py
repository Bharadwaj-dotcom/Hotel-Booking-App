...
from accounts.utils import generateRandomToken, generateSlug, sendEmailToken, sendOTPtoEmail
...
def verify_email_token_user(request, token):
    try:
...
def login_vendor(request):    
    if request.method == "POST":
        ...
        hotel_user_qs = HotelVendor.objects.filter(email=email)
        ...
        print("Email:", hotel_user.email)
        print("Verified:", hotel_user.is_verified)
...
def verify_email_token_vendor(request, token):
    try:
        ...
        hotel_user.save()
        messages.success(request, "Email verified successfully!")
        return redirect(reverse('login_vendor'))  # Better than hardcoding the path
    except HotelUser.DoesNotExist:
        return HttpResponse("Invalid Token")
